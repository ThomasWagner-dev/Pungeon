...
public class Weapon extends Item {
    ...
    public final AdvancedImage img;
    ...
    public Weapon(String name, String displayname, String description, int range, int dmg, String dmgType, int speed, int cooldown, String spriteName, double scale, String hitbox, boolean isMelee, int angle) {
        ...
        this.angle = angle;
        img = DungeonWorld.scaleImage(Utils.loadImageFromAssets(spriteName), scale);
    }
    ...
    /**
     * returns an extick clone of the weapon
     ...
     */
    public void attack(DungeonWorld world, WorldObj owner, double[] direction) {
        if (!checkCooldown()) return;
        if (owner instanceof Enemy) {
            Enemy e = (Enemy) owner;
            int cap = 0;
            if (e instanceof Collider) cap = 1;
            if (e.objectsOfInterfaceAtOffset(0, 0, Collider.class).size() != cap) return;
        }
        resetCooldown();
        Projectile p;
        ...
        world.addObject(p, owner.x, owner.y);
